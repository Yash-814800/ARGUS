...
class RCAEngine(BaseAgent):
    ...
    async def handle_anomaly(self, msg: AgentMessage):
        ...
        try:
            ...
            incident, created = await self.correlator.correlate(payload, correlation_id=msg.correlation_id)
            ...
            if created or incident.status == "detecting" or (incident.status == "diagnosing" and incident.diagnosis is None):

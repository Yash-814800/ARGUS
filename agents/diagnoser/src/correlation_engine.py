...
class CorrelationEngine:
    ...
    async def correlate(self, anomaly_data: Dict[str, Any], correlation_id: Optional[str] = None) -> Tuple[Incident, bool]:
        """
        ...
        try:
            async with get_session() as session:
                ...
                incident, created = await self._find_or_create_incident(session, service, correlation_id)
            ...
            logger.warning(f"IntegrityError during correlation (probably race condition with orchestrator): {e}. Retrying to load incident.")
            async with get_session() as session:
                if correlation_id:
                    stmt = select(Incident).where(Incident.id == correlation_id)
    ...
    async def _find_or_create_incident(self, session: AsyncSession, service: str, correlation_id: Optional[str] = None) -> Tuple[Incident, bool]:
        """
        Finds an active incident within the time window that is topologically or temporally related,
        or creates a new one.
        """
        # First check if an incident with this exact ID already exists
        if correlation_id:
            stmt = select(Incident).where(Incident.id == correlation_id)
            result = await session.execute(stmt)

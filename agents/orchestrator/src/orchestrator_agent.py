...
class OrchestratorAgent(BaseAgent):
    ...
    def _transition_fsm(self, fsm: IncidentFSM, target_state: str) -> None:
        """Transition FSM step-by-step to target_state to avoid invalid transition errors."""
        path = ["detecting", "diagnosing", "proposing_remediation", "safety_review", "executing", "verifying", "resolved"]
        if target_state not in path or fsm.state not in path:
            fsm.transition(target_state)
            return
            
        current_idx = path.index(fsm.state)
        target_idx = path.index(target_state)
        
        if target_idx > current_idx:
            for i in range(current_idx + 1, target_idx + 1):
                fsm.transition(path[i])
        else:
            fsm.transition(target_state)
    ...
    async def _handle_anomaly(self, msg: AgentMessage) -> None:
        ...
        self._transition_fsm(fsm, "diagnosing")
    ...
    async def _handle_diagnosis_result(self, msg: AgentMessage) -> None:
        ...
        payload = msg.payload
        real_diagnosis = payload.get("diagnosis", {})
        try:
            self._transition_fsm(fsm, "proposing_remediation")
        ...
        async with get_session() as session:
            ...
            if incident:
                ...
                incident.diagnosis = real_diagnosis
                incident.diagnosis_confidence = payload.get("confidence", 0)
                incident.root_cause_category = real_diagnosis.get("root_cause_category")
                incident.root_cause_service = real_diagnosis.get("root_cause_service")
    ...
    async def _handle_safety_decision(self, msg: AgentMessage) -> None:
        ...
        if status == "approved":
            try:
                self._transition_fsm(fsm, "executing")
            except ValueError:
                return
            ...
            try:
                self._transition_fsm(fsm, "safety_review")
            except ValueError:
                pass
            await self._update_incident_timeline(incident_id, "safety_approved",
    ...
    async def _handle_execution_result(self, msg: AgentMessage) -> None:
        ...
        if status == "success":
            try:
                self._transition_fsm(fsm, "verifying")
                self._transition_fsm(fsm, "resolved")
            except ValueError as e:
                logger.error("FSM transition error: %s", e)
                return

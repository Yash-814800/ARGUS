...
class SyntheticProber(BaseAgent):
    ...
    def __init__(self, nats_url: str):
        ...
        self.http_client = httpx.AsyncClient(timeout=10.0, follow_redirects=True) # Longer timeout for complex flows
        self.poll_interval = 30 # Run simulation every 30 seconds
        
        # Route through the NGINX API Gateway since users hit that
        self.gateway_url = os.getenv("GATEWAY_URL", "http://api-gateway:8000")
    ...
    async def simulate_checkout_flow(self):
        ...
        try:
            ...
            # 2. Search Products
            search_resp = await self.http_client.get(f"{self.gateway_url}/search?q=laptop")
